...
def draw_boxes2(img, v_boxes, v_labels, v_scores):
	for i in range(len(v_boxes)):
		...
		# Prints the text.    
		img = cv2.rectangle(img, (x1, y1 - 10), (x1 + w, y1), color, -1)
...
def load_yolo_with_weights():
	# define the yolo v3 model
	yolov3 = make_yolov3_model()
	# load the weights
	weight_reader = WeightReader('../saved_models/yolov3.weights')
	# set the weights
...
def yolo_predict(yolov3,frame,plot=True):
	...
	image = cv2.resize(frame, (input_h,input_w), interpolation = cv2.INTER_AREA)
	image = image.astype('float32')
	image /= 255.0
    # add a dimension so that we have one sample
	image = expand_dims(image, 0)
...
def main():
	...
	file_name = '../saved_models/img2.jpg'
	# define the expected input shape for the model
	input_w, input_h = 320, 320

	image, image_w, image_h = load_image_pixels(file_name, (input_w, input_h))
	...
	# draw what we found
	draw_boxes(file_name, v_boxes, v_labels, v_scores)
