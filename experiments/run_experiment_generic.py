...
Fault_injection_Report = make_dataclass("Fault_injection_Report", [("model_name",str),("layer_name",str), ("num_of_injection", int), ("misc_clip", int),("misc_rang", int),("clip_robustness",int),("range_robustness",int)])

Models_statistics = make_dataclass("Models_statistics", [("model_name",str),("dataset",str), ("accuracy",float)])

def generate_layer_report(model_name,model,inj_model,DATASET,CLASSES,RANGER,experiment_name,layer_name,NUM_SAMPLE_ITERATION):
    ...
    for idx in progress_bar:
            ...
            if np.argmax(pred) == y:
                ...
                x_batch,y_batch = gen_batch(x,y,batch_size=NUM_SAMPLE_ITERATION)
    ...
    line_report = Fault_injection_Report(model_name,layer_name,tot_samples,tot_clip_misc,tot_thres_misc,clip_rob,thresh_rob)
    return line_report


def generate_report(model_name,model,DATASET,experiment_name,NUM_SAMPLE_ITERATION,RESUME_LAYER,START_IDX):
    ...
    RANGER,CLASSES = add_ranger_classes_to_model(model,layer_names,NUM_INJECTIONS=60)
    ...
    RANGER.tune_model_range(x_train,verbose=True)
    ...
    for layer in layer_names:
        line = generate_layer_report(model_name,model,inj_model,DATASET,CLASSES,RANGER,experiment_name,layer,NUM_SAMPLE_ITERATION)
        report = [line]
...
if __name__ == '__main__':
    ...
    #Load chosen dataset
    x_train,x_val,y_train,y_val,NUM_CLASSES = load_dataset(DATASET=DATASET_NAME)

    #Extract numclasses and input shape from dataset
    INPUT_SHAPE = x_train[0].shape

    #Load model and preprocess function
    model,preprocess_fn = load_model(MODEL=MODEL,NUM_CLASSES=NUM_CLASSES,INPUT_SHAPE=INPUT_SHAPE)
    ...
    if DO_TRAIN:
        train_model(model,DATASET,NUM_CLASSES,EPOCHS)
    ...
    if DO_REPORT:
        generate_report(MODEL,model,DATASET,experiment_name=EXP_NAME,NUM_SAMPLE_ITERATION=128,RESUME_LAYER=RESUME_LAYER,START_IDX=START_AT)
