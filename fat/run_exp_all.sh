...
train_all_model(){
    $LAYER=$1
    #PREFAT
    ARGS="--layer $1 --experiment_name PRE_FAT_SINGLE_LAYER_$1 --prefat"
    python post_fat_report.py $ARGS

    for freq in 1.0 0.5 0.75
        do              
            #VANILLA
            ARGS="--checkpoint FREQUENCY_${freq}_SINGLE_LAYER_$1 --experiment_name FREQUENCY_${freq} --epoch 20 --layer $1  --root ./results/batch_global/$1"
            python post_fat_report.py $ARGS

            #GOLDEN
            ARGS="--checkpoint GOLDEN_FREQUENCY_${freq}_SINGLE_LAYER_$1 --experiment_name GOLDEN_${freq} --epoch 20 --layer $1  --root ./results/batch_global/$1"
            python post_fat_report.py $ARGS

            #MIXED v1  alternate a livello di batch
            ARGS="--checkpoint MIXED_${freq}_SINGLE_LAYER_$1 --experiment_name MIXED_V1_${freq} --epoch 20 --layer $1  --root ./results/batch_global/$1"
            python post_fat_report.py $ARGS

            #MIXED v3 alternate ma ricreiamo le label golden con i modello in maniera progressiva
            ARGS="--checkpoint MIXED_V3_${freq}_SINGLE_LAYER_$1 --experiment_name MIXED_V3_${freq} --epoch 60 --layer $1 --root ./results/batch_global/$1"
            python post_fat_report.py $ARGS
