...
def post_fat_segmentation_report(injection_point,out_prefix="PREFAT_NEW", prefat=False,SINGLE_F1_FILE=False,SKIP_INJECTION=False):
    ...
    model = tf.keras.models.load_model("../saved_models/unet_pet_classic")
    ...
    x_train,x_val,y_train,y_val = pet.load_data((128,128))
    ...
    def range_tune(RANGER):
        ...
        if not SKIP_INJECTION:
            RANGER.tune_model_range(x_train, reset=False,verbose=False)
    ...
    progress_bar = tqdm(zip(x_val[:150],y_val[:150]),total=150)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--layer"      , action = "store",default="batch_normalization_16")   

    args            = parser.parse_args()
    layer          = args.layer

    post_fat_segmentation_report(injection_point=layer,out_prefix="UNET_PET")